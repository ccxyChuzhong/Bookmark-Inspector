...
[![Version](https://img.shields.io/badge/version-v3.0.0-c0614a.svg)]()
[![Edge Version](https://img.shields.io/badge/Edge-v3.0.0-0078D7?logo=microsoftedge&logoColor=white)](https://microsoftedge.microsoft.com/addons/detail/%E4%B9%A6%E7%AD%BE%E7%AE%A1%E7%90%86%E5%99%A8/clnomppfflgjbhmpabhknaimdndgjmhd)

一款专注于书签健康管理的浏览器扩展，提供书签导入导出、失效检测、代理重试与多线程配置。

> 🎉 **v3.0.0** 全面焕新：暖色调 UI 主题、树形文件夹下拉、紧凑配置页、书签项行内删除、状态徽标完善。

📚 **文档导航**：[功能演示 DEMO](./DEMO.md) · [升级指南 UPGRADE](./UPGRADE.md)
...
- **状态徽标**：书签列表对每条 URL 直观展示 `可用 / 失效 / 检测中 / 未检测` 状态。
- **进度回显**：检测过程中实时显示进度条和百分比。
...
- **智能导入模式**：（新增√）
  ...
  - **保持结构**：完整保持原有文件夹层次结构
- **文件夹选择**：可选择导入到任意书签文件夹（树形下拉，支持折叠）（新增√）
- **直观设置界面**：主面板右上角齿轮会打开独立的 `settings.html`，用于配置代理信息与线程数。
- **WebDAV 同步**：支持通过标准 WebDAV 协议备份/恢复书签到网盘。
- **跨浏览器迁移**：一键转换Chrome↔Firefox↔Edge格式 （计划中）

### 🧹 书签管理
- **行内删除**：书签列表每一项支持悬停浮现的图标按钮，一键删除单条书签。
- **批量操作**：在检测结果区可对失效书签进行复制/打开/删除。
- 自动分类建议（基于链接内容分析）（计划中）

### 🎨 界面与体验（v3.0 焕新）
- **暖色调主题**：基于"Warm Linen"配色，弱化视觉疲劳。
- **紧凑配置页**：设置项垂直密度优化，可在一屏内查看代理 / 并发 / WebDAV 全部配置。
- **树形文件夹下拉**：文件夹下拉框改为可折叠的树形结构，便于在多层级中快速定位。
...
### 核心操作
1. **阅读主面板**：顶栏含导出、导入、检测按钮，以及文件夹选择（树形下拉）；导入按钮会弹出设置层，用于选择目标文件夹和导入模式。
2. **浏览书签列表**：选择文件夹后下方列出该文件夹的书签，每条书签显示状态徽标，行尾悬停时浮现删除图标。
3. **查看检测结果**：进度条和百分比会在检测过程中实时更新；结果列表仅显示失效书签，同时提供新标签/弹窗/复制/删除等快捷操作。
4. **设置页配置**：点击右上角齿轮后打开 `settings.html`，可将多线程数量调至 1~20，并在直连失败时启用代理（支持 HTTP/HTTPS/SOCKS5），亦可配置 WebDAV 同步。
5. **恢复与持久化**：检测过程中关闭弹窗后重新打开时会尝试从 `localStorage` 恢复状态，并继续后台检测。
...
- 验证状态徽标：未检测过的书签应显示"未检测"；检测中应显示"检测中"；检测完成后根据结果显示"可用 / 失效"。
- 验证行内删除：在书签列表中悬停一条书签，行尾应浮现垃圾桶图标，点击后确认对话框，确认后该项立即从列表移除。
- 验证代理：在设置页开启代理、填写有效信息，运行检测并人为阻断直连，确认自动进入代理阶段并在结束后恢复原始代理设置。
- 验证线程配置：将"并发检测数量"设为 5、15 等不同值，保存后重新检测，确保批次大小符合设置。
- 验证 WebDAV：在设置页填写 WebDAV 服务器信息，依次测试"测试连接 / 查询远端 / 备份 / 恢复"四个操作。
...
- 书签列表中文件夹项不显示状态徽标与删除按钮，仅对真正的书签项生效。
